...
public class EnumMapDrawer<ET> : PropertyDrawer where ET : System.Enum
{
    bool folded = false;
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (folded)
        {
            ...
            GUI.Label(new Rect(position.x + labelWidth, position.y + 24, position.width - labelWidth, EditorGUIUtility.singleLineHeight + 8), "folded", "ChannelStripSendReturnBar");
        }
        else
        {
            ...
            else
            {
                ...
                for (int i = 0; i < enums.Length; ++i)
                {
                    ...
                    GUI.Button(preRect, GUIContent.none, "Radio");
                    GUI.Box(outerRect, GUIContent.none, "window");
                    OnItemGUI(propRect, list.GetArrayElementAtIndex(i), new GUIContent(enums[i]), i);
                }
    ...
    {
        EditorGUI.PropertyField(propRect, item, label);
    }
}
